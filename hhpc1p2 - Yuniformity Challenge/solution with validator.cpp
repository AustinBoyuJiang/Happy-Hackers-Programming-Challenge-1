...
int n,q,a[N],pos[A],lst[N],eq0[N],eq[N];

inline void SOLVE(int Case){
	in_r.readInts(n,q);
	assert(n>=1&&n<=5e3);
	assert(q>=1&&q<=5e3);
	Vec<ll> aa=in_r.readIntArray(n);
	rep(i,1,n){
		a[i]=aa[i-1];
		assert(a[i]>=-1e6&&a[i]<=1e6);
		pos[a[i]+(int)1e6]=i;
		lst[i]=pos[(int)1e6-a[i]];
		chkmax(lst[i],lst[i-1]);
		eq0[i]==a[i]==0;
		eq[i]=a[i]==a[i-1];
		eq0[i]+=eq0[i-1];
		eq[i]+=eq[i-1];
	...
	rep(i,1,q){
		...
		assert(l<=r);
		assert(r<=n);
		if(lst[r]>=l||eq0[r]-eq0[l-1]||eq[r]-eq[l]==r-l){
			cout<<"YES"<<endl;
		}
		else{
			cout<<"NO"<<endl;
