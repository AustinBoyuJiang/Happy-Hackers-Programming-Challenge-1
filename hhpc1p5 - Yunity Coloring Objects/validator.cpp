...
void SETUP(){
	...
	#endif
	#ifdef READLOCAL
	freopen("data/hhpc1p1.1.in","r",stdin);
...
string getLine(){
	string line;
	getline(cin,line);
	assert(line.empty()||line[line.size()-1]!=' ');
	return line;
}
...
Vec<ll> splitLine(string line,ll bound=INF){
	line+=' ';
	Vec<ll> nums;
	ll num=0;
	int sign=1,flag=0;
...
inline void SOLVE(int Case){
	...
	rep(i,1,q){
		...
		assert(b>=1&&b<=n);
		assert(a!=b);
//		assert(flag[make_pair(a,b)]==0);
//		flag[make_pair(a,b)]=1;
//		flag[make_pair(b,a)]=1;
	}
	end();
}
