...
int st[C],ed[C],len[C];
pair<PID,int> lst[C][K+2];
PID invlst[C][K+2];

void lstadd(int i,pair<PID,int> a,PID b){
	lst[i][ed[i]]=a;
	invlst[i][ed[i]]=b;
	ed[i]=(ed[i]+1)%(K+2);
	len[i]++;
...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		for(int j:p[i]){
			PID val=mult(lst[j][(ed[j]-1+K+2)%(K+2)].fir,j);
			lstadd(j,{val,i},{inv(val.fir),-val.sec});
			if(len[j]>K+1){
				st[j]=(st[j]+1)%(K+2);
				len[j]--;
			}
			rep(pre,1,K){
				if(len[j]<pre+1) break;
				int pos=lst[j][(ed[j]-pre+K+2)%(K+2)].sec;
				PID val=mult(lst[j][(ed[j]-1+K+2)%(K+2)].fir,invlst[j][(ed[j]-pre-1+K+2)%(K+2)]);
				sq[pre].upd(pos,val);
			}
		}
		for(int id:q[i]){
