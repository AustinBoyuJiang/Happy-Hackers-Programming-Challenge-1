...
inline void SOLVE(int Case){
	...
	rep(i,1,n){
		rep(j,1,n){
			...
			ld pos=(a[j]-a[i])*x/(b[i]-a[i]-b[j]+a[j]);
			if(b[i]-a[i]<b[j]-a[j]) u[i].pb({pos,c[j]});
			if(b[i]-a[i]>b[j]-a[j]) d[i].pb({pos,c[j]});
		}
		sort(all(query[i]));
		sort(all(u[i]));
		sort(all(d[i]));
	...
	rep(i,1,n){
		...
		for(pair<ld,int> dd:d[i]) res+=dd.sec;
		for(PI ask:query[i]){
